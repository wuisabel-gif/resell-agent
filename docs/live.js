...
if (root) {
    ...
    const card = (listing) => {
        ...
        if (heading)
            heading.textContent = listing.platform ?? "";
        if (price)
            price.textContent = money(listing.price);
        if (copy)
            copy.textContent = `${listing.title ?? ""}\n\n${listing.description ?? ""}`;
        return el;
    };
    const retailBlock = (rows) => {
    ...
    fetch(`${apiBase}/api/health`)
        .then((res) => res.json())
        .then((body) => {
        if (body && body.ok && body.ready)
            setStatus("Live model is ready. One photo, then draft.", "ok");
        else
            setStatus("The live model is up, but the key is not set yet.", "warn");
    })
        .catch(() => {
